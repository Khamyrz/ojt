...
class InternController extends Controller
{
    ...
    public function store(Request $request)
    {
            ...
            try {
                ...
                if (is_array($decoded) && isset($decoded['exp']) && isset($decoded['invited_by'])) {
                    if ($decoded['exp'] >= now()->timestamp) {
                        ...
                        return redirect()->route('intern.login')->withErrors(['invited_token' => 'Expired Link'])->withInput();
                    }
                }
            } catch (\Throwable $e) {
        ...
        // Redirect to intern login page with OTP modal trigger
        $redirect = redirect()->route('intern.login')->with('success', "Registration successful! Please verify your email with the OTP code sent to your email address.")
    ...
    /**
     * Show a single intern's details (returns JSON for modal).
     */
    ...
    /**
     * Show the form for editing an intern (returns JSON for modal).
     */
